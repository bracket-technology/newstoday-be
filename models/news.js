...
module.exports = {
  ...
  getAllNews: (search, orderBy, limit, offset) => {
    return new Promise((resolve, reject) => {
      db.query(`SELECT n.newsId,u.name,u.userImage,c.categoryName,n.title,n.descriptionNews,n.content,n.newsImage,n.created_at FROM news n JOIN users u ON n.userId=u.userId JOIN category c ON n.categoryId=c.categoryId WHERE title LIKE '%${search}%' ORDER BY title ${orderBy} LIMIT ${limit} OFFSET ${offset}`, (err, result) => {
        if (err) {
          reject({
            success: false, message: err.sqlMessage, data: {
              errCode: err.code, errNo: err.errno
